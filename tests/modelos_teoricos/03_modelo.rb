...
def simular(nro_corrida, lambda, mu, k)
...
$mu = ARGV[1].to_f
$k = ARGV[2].to_i
$archivo = "m_m_#{$k}_#{"%.0f"%$lambda}_#{"%.0f"%$mu}.txt"

#principal

$clientes_por_corrida = 26000
...
$corrida = 1     
while $corrida <=120#!converge para[:ls_real]
  simular($corrida, $lambda, $mu, $k)   
  File.open($archivo, "a") { |archivo| 
    ...
    archivo.puts "nro_corridas|lambda|mu|k|lq|ls|wq|ws" if $corrida == 1
    archivo.puts "#{$corrida}|#{$lambda}|#{$mu}|#{$k}|#{$recolector_lq.mean}|#{$recolector_ls.mean}|#{$recolector_wq.mean}|#{$recolector_ws.mean}"      
  }
  $corrida += 1
end                                      
