...
def traza(): 
	if corrida == 1:
		salida_traza.write("%.2f|%s|%i|%i|%i\n"%(now(),modo,len(cola_control),len(empaque.waitQ)+len(empaque.activeQ),len(almacen)))
...
salida.write("max_cola_control_calidad|promedio_cola_despacho|max_cola_despacho|")
